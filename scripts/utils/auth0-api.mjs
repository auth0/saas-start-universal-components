...
function isAuthError(e) {
  ...
  if (
    ...
    (stderr.includes("expired") || stderr.includes("invalid"))
  ) {
    return true
  }
 ...
 */
export async function auth0ApiCall(
  method,
  endpoint,
  data = null,
  ...
  try {
    ...
    if (isAuthError(e)) {
      throw new Error(
        `Authentication failed. Your Auth0 session may have expired.`
      )
    }
    // For scope errors, return null gracefully (the feature may not be available)
    if (
      e.stderr?.includes("lacks scope") ||
      e.stderr?.includes("insufficient_scope")
    ) {
      console.warn(
